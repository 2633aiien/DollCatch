...
class MoreFollowViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    var moreMInt = 0
    var moreSInt = 0
    var followShopArr : [FollowShopMachine] = []
    var followMachineArr : [FollowShopMachine] = []
...
//        moreMInt += 1
    ...
    func parseMoreMachineJson(data: Data) {
        ...
        let jsonArray = try JSONSerialization.jsonObject(with: data, options: []) as! [Any]
            for jsonResult in jsonArray {
                ...
                //Add it to the array
                followMachineArr.append(machine)
                addAnnotation()
            }
        }
        catch {
    ...
    func addAnnotation() {
        ...
        if self.segmentControl.selectedSegmentIndex == 1 {
            for i in 0...self.followMachineArr.count-1 {
                var objectAnnotation = MKPointAnnotation()
                objectAnnotation.coordinate = CLLocation(
                    latitude: Double(self.followMachineArr[i].latitude) ?? 0,
                    longitude: Double(self.followMachineArr[i].longitude) ?? 0).coordinate
                objectAnnotation.title = self.followMachineArr[i].title
            ...
            for i in 0...self.followShopArr.count-1 {
                var objectAnnotation = MKPointAnnotation()
                objectAnnotation.coordinate = CLLocation(
                    latitude: Double(self.followShopArr[i].latitude) ?? 0,
                    longitude: Double(self.followShopArr[i].longitude) ?? 0).coordinate
                objectAnnotation.title = self.followShopArr[i].title
                //            objectAnnotation.subtitle =
                //              "艋舺公園位於龍山寺旁邊，原名為「萬華十二號公園」。"
                self.myMapView.addAnnotation(objectAnnotation)
            }
