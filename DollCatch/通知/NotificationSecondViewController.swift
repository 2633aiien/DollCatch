...
import UIKit
import CoreData
class NotificationSecondViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getNotification() {
        let url = URL(string: "https://www.surveyx.tw/funchip/get_notify_machine.php")!
    ...
    func getNoti(id: String) {
            let url = URL(string: "https://www.surveyx.tw/funchip/reset_notify_machine.php")!
            var request = URLRequest(url: url)
            request.httpMethod = "POST"
            let json: [String: Any] = [
                "userId": "\(userData[0].userId)",
